...
class OdPairStream(sparkSession: SparkSession) extends StreamCompute {
  ...
  override def compute(): Unit = {
    ...
    // 重新分区
    val odFrame = sparkSession.sql(odPairSql).repartition(40)
    val odFrameQuery = odFrame.withWatermark("inTime", odPairDelay)
      .writeStream.foreachBatch((batch: DataFrame, batchId: Long) => {
      //      获取配对数据
      batch.persist()
      val odPairFrame = batch.select("in_station", "inTime", "out_station", "outTime")
      val odCountFrame = odPairFrame
...
object OdPairStream {
  def main(args: Array[String]): Unit = {
    val sparkSession = SparkSession.builder().appName("OdPairStream").getOrCreate()
    sparkSession.sparkContext.setLogLevel("WARN")
