...
class BaseCalculate(baseDataLoad: BaseDataLoad, sectionTravelGraph: SectionTravelGraph) extends AdjustParam[Double] {
  ...
  val pathMathCache = new LRUCacheHashMap[(String, String), util.List[Path]](20000, true)

  def getLegalPathList(kspNum: Int, odWithTime: OdWithTime): java.util.List[Path] = {

    //    一定要去除首尾有换乘的站点,这样的路径才是合法路径
    val pathList = pathMathCache.get((odWithTime.getInId, odWithTime.getOutId))
    val legalPath = if (pathList != null) {
      pathList
    } else {
      // 计算路径
      val graph: Graph = baseDataLoad.getGraph
      val pathComputeService = new PathService(kspNum, graph, sectionWithDirectionMap)
      val pathList = pathComputeService.getLegalKPath(odWithTime.getInId, odWithTime.getOutId)
      pathMathCache.put((odWithTime.getInId, odWithTime.getOutId), pathList)
      pathList
    }
    legalPath
  }
  ...
  def dynamicCalculate(odWithTime: OdWithTime, staticCost: util.Map[Path, Double], minGeneralizedCost: (Path, Double), legalPath: util.List[Path],
    ...
    val logitResult = Logit.logit(dynamicCost, minGeneralizedCost, odWithTime.getPassengers)
    logitResult
  }
